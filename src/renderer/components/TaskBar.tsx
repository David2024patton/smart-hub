...
export function TaskBar() {
  ...
  const handleAppClick = (winId: string) => {
    ...
    if (!win) return
    if (win.minimized) { restoreWindow(winId) }
    else if (win.id === activeWindowId) { minimizeWindow(winId) }
  ...
  return (
    <>
      ...
      <div className="flex items-center h-[44px] px-2 shrink-0 select-none"
        ...
        <div className="flex-1 flex items-center gap-0.5 overflow-x-auto">
          {windows.map((win: DesktopWindow) => (
            <button key={win.id}
              ...
              style={{
                background: win.id === activeWindowId && !win.minimized ? 'var(--accent-subtle)' : 'rgba(255,255,255,0.02)',
                color: win.id === activeWindowId ? 'var(--accent)' : 'var(--text-muted)',
                border: win.id === activeWindowId && !win.minimized ? '1px solid hsla(160, 84%, 39%, 0.12)' : '1px solid transparent',
              }}
        ...
        {/* Clock */}
        <div className="flex items-center gap-1.5 text-xs font-mono ml-auto shrink-0" style={{ color: 'var(--text-muted)' }} data-tooltip={dateStr}>

...
export function DashboardContent() {
  const { openMaximized } = useDesktop()
    ...
    <div className="p-6 space-y-6 overflow-auto h-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="stat-card animate-fade-up cursor-pointer" data-color="teal"
          onClick={() => openMaximized('projects')} data-tooltip="View projects">
        ...
        <div className="stat-card animate-fade-up cursor-pointer" data-color="amber"
          onClick={() => openMaximized('kanban')} data-tooltip="View tasks">
          <div className="flex items-start justify-between">
        ...
        <div className="stat-card animate-fade-up cursor-pointer" data-color="blue"
          onClick={() => openMaximized('rag-lab')} data-tooltip="Open RAG Lab">
          <div className="flex items-start justify-between">
        ...
        <div className="lg:col-span-3 glass-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-mono px-2.5 py-1 rounded-full cursor-pointer hover:opacity-80"
              ...
              onClick={() => openMaximized('mcp-grid')} data-tooltip="Open MCP Mesh">
              {MCP_SERVERS.reduce((s, v) => s + v.tools, 0)} tools
            </span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {MCP_SERVERS.map(s => (
              <div key={s.name} className="mcp-node cursor-pointer" onClick={() => openMaximized('mcp-grid')}>
          ...
          <div className="flex flex-wrap gap-2 mt-4 pt-4" style={{ borderTop: '1px solid var(--glass-border)' }}>
            <button className="action-chip action-chip-primary" onClick={() => openMaximized('projects')} data-tooltip="Create a new project">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
              Create Project
            </button>
            <button className="action-chip" onClick={() => openMaximized('terminal')} data-tooltip="Open terminal">
            ...
            <button className="action-chip" onClick={() => openMaximized('rag-lab')} data-tooltip="Ingest into RAG">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
              RAG Ingest
            </button>
            <button className="action-chip" onClick={() => openMaximized('security')} data-tooltip="Open security shield">

...
/* ──────────────────────────────────────────────
   Mock data: in production these come from Tauri IPC
   ────────────────────────────────────────────── */
          ...
          <div className="relative z-10 space-y-8">
            ...
            <div className="animate-fade-up">
              ...
              <p className="mt-1 text-sm" style={{ color: 'var(--text-muted)' }}>
                Sovereign AI Orchestration OS &middot; v0.1.0-alpha &middot; Phase 0.1
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 stagger-children">
              ...
              >
                ...
                <div className="stat-bar-track">
                  <div className="stat-bar-fill" style={{ width: '15%' }}></div>
                </div>
              </div>
              ...
              <div 
                className="stat-card animate-fade-up cursor-pointer" 
                data-color="blue"
                onClick={() => setActivePage('rag-lab')}
              >
                <div className="flex items-start justify-between">
                  <div>
                    <p className="stat-label">RAG Sources</p>
                ...
                <div className="stat-bar-track">
                  <div className="stat-bar-fill" style={{ width: '52%' }}></div>
                </div>
              </div>
              ...
              <div className="stat-card animate-fade-up" data-color="violet">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="stat-label">System Load</p>
                    <p className="stat-value">12<span style={{ fontSize: '1.25rem', fontWeight: 400, opacity: 0.6 }}>%</span></p>
                  </div>
                  <div className="stat-icon">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div className="stat-bar-track">
                  <div className="stat-bar-fill" style={{ width: '12%' }}></div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
              ...
              {/* Left: MCP Mesh Status (3 cols) */}
              <div className="lg:col-span-3 glass-card p-6 animate-fade-up" style={{ animationDelay: '0.25s' }}>
                <div className="flex items-center justify-between mb-5">
                  <div>
                    <h2 className="text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>MCP Mesh Status</h2>
                    <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>
                      {MCP_SERVERS.filter(s => s.status === 'active').length} of {MCP_SERVERS.length} servers active
                    </p>
                  </div>
                  <span
                    className="text-xs font-mono px-2.5 py-1 rounded-full cursor-pointer hover:opacity-80 transition-opacity"
                    style={{
                      background: 'var(--accent-subtle)',
                      color: 'var(--accent)',
                      border: '1px solid hsla(160, 84%, 39%, 0.15)'
                    }}
                    onClick={() => setActivePage('mcp-grid')}
                  >
                    {MCP_SERVERS.reduce((sum, s) => sum + s.tools, 0)} tools
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {MCP_SERVERS.map((server) => (
                    <div 
                      key={server.name} 
                      className="mcp-node cursor-pointer"
                      onClick={() => setActivePage('mcp-grid')}
                    >
                      <span className={`mcp-dot ${server.status === 'active' ? 'mcp-dot-active' : 'mcp-dot-idle'}`} />
                      <span className="flex-1 font-mono text-xs truncate" style={{ color: 'var(--text-secondary)' }}>
                        {server.name}
                      </span>
                ...
                {/* Quick Actions Row */}
                <div className="flex flex-wrap gap-3 mt-6 pt-5" style={{ borderTop: '1px solid var(--glass-border)' }}>
                  <button 
              ...
              <div className="lg:col-span-2 glass-card p-6 animate-fade-up" style={{ animationDelay: '0.3s' }}>
                <div className="flex items-center justify-between mb-5">
                  ...
                  {RECENT_ACTIVITY.map((item, idx) => (
                    <div key={item.id} className="activity-item">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs mt-0.5 font-mono" style={{ color: 'var(--text-ghost)' }}>
                          {item.time}
                        </p>
                      </div>
                    </div>
